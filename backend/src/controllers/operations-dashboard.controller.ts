...
export const operationsDashboardController = {
  summary: asyncHandler(async (req, res: Response) => {
    const summary = await dashboardEngineService.getWidget('operations', { range: parseDateRange(req.query) });
    return sendSuccess(res, 200, { message: 'Operations dashboard summary fetched', data: summary });
  }),
};
