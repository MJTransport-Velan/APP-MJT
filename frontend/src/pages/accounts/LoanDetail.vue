  ...
  <div>
    <div class="d-flex flex-wrap align-center justify-space-between mb-4 ga-2">
      <div class="d-flex align-center ga-2">
        <AppBtn icon="mdi-arrow-left" variant="text" size="small" title="Back to Loans & EMI" @click="goBack" />
        <div>
          <h2 class="text-h6 mb-1">{{ loan ? loan.loanName : 'Loan' }}</h2>
          <p v-if="loan" class="text-caption text-medium-emphasis mb-0">
            {{ loan.loanNumber }} · {{ loanTypeLabel(loan.loanType) }} · {{ loan.lenderName }}
            <template v-if="loan.vehicle"> · {{ loan.vehicle.registrationNumber }}</template>
            <template v-else-if="loan.capitalPartner"> · {{ loan.capitalPartner.name }}</template>
          </p>
        </div>
      </div>
      <AppChip v-if="loan" :color="loanStatusColor(loan.status)">{{ loan.status }}</AppChip>
    </div>
    ...
    <template v-else-if="loan">
      ...
      <div class="row">
        <div class="col-12 col-sm-6 col-md-3">
          <ProfitCard label="Total Loan" :value="loan.principalAmount" icon="mdi-bank-outline" color="primary" />
        </div>
        <div class="col-12 col-sm-6 col-md-3">
          <ProfitCard label="Outstanding Principal" :value="loan.totals.outstandingPrincipal" icon="mdi-hand-coin-outline" color="warning" />
        </div>
        <div class="col-12 col-sm-6 col-md-3">
          <ProfitCard label="Principal Paid" :value="loan.totals.principalPaid" icon="mdi-cash-minus" color="success" />
        </div>
        <div class="col-12 col-sm-6 col-md-3">
          <ProfitCard label="Interest Paid" :value="loan.totals.interestPaid" icon="mdi-percent-outline" color="info" />
        </div>
      </div>

      <AppCard class="pa-4 mt-3">
        <div class="row row-dense">
          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">EMI Amount</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ formatCurrency(loan.emiAmount) }}</div>
          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Interest Rate</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ loan.interestRatePercent }}% p.a.</div>

          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Total EMI Paid</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ formatCurrency(loan.totals.totalEmiPaid) }}</div>
          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Paid / Remaining</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ loan.totals.paidCount }} / {{ loan.totals.remainingEmis }} of {{ loan.tenureMonths }}</div>

          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Next EMI Date</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ loan.totals.nextEmiDate ? formatDate(loan.totals.nextEmiDate) : '—' }}</div>
          <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Loan Closing Date</div>
          <div class="col-6 col-md-3 text-body-2 font-weight-medium">{{ loan.totals.loanClosingDate ? formatDate(loan.totals.loanClosingDate) : '—' }}</div>

          <template v-if="loan.totals.overdueCount > 0">
            <div class="col-6 col-md-3 text-body-2 text-medium-emphasis">Overdue EMI</div>
            <div class="col-6 col-md-3 text-body-2 font-weight-medium text-error">{{ loan.totals.overdueCount }}</div>
          </template>
        </div>
      </AppCard>
      ...
      <AppCard class="pa-4 mt-4">
        ...
        <div class="tblwrap">
          <AppTable density="compact">
            <thead>
              <tr>
                <th>EMI No.</th><th>Due Date</th>
                <th class="text-right">EMI</th><th class="text-right">Principal</th><th class="text-right">Interest</th>
                <th>Status</th><th>Paid On</th><th>Reference</th><th class="text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="i in loan.installments" :key="i.id">
                ...
                <td><AppChip size="x-small" :color="emiStatusColor(i.status)">{{ i.status }}</AppChip></td>
                <td>{{ i.paidDate ? formatDate(i.paidDate) : '—' }}</td>
                <td>{{ i.referenceNumber || '—' }}</td>
                <td class="text-right">
                  <AppBtn v-if="i.status !== 'PAID' && i.status !== 'WAIVED'" size="small" variant="tonal" color="primary" @click="openPayDialog(i)">Pay</AppBtn>
                  <AppBtn v-else-if="i.status === 'PAID'" size="small" variant="text" color="error" @click="onReverse(i)">Reverse</AppBtn>
                </td>
              </tr>
            </tbody>
          </AppTable>
    ...
    <!-- ------------------------------------------------------ Pay EMI -->
    <MasterFormDialog v-model="payDialog" title="Pay EMI" :loading="paying" @submit="onPay">
      <template v-if="payTarget && loan">
        <p class="text-caption text-medium-emphasis mb-3">
          Saving this debits the payment account, reduces the loan outstanding and records the Financial Entry automatically.
        </p>

        <div class="d-flex ga-2">
          <AppTextField :model-value="loan.vehicle?.registrationNumber || '—'" label="Vehicle" readonly density="compact" class="mb-2 flex-1-1" />
          <AppTextField :model-value="loan.loanNumber" label="Loan" readonly density="compact" class="mb-2 flex-1-1" />
        </div>
        <div class="d-flex ga-2">
          <AppTextField v-model="payForm.paidDate" type="date" label="EMI Date" class="mb-2 flex-1-1" />
          <AppTextField v-model.number="payForm.paidAmount" type="number" label="EMI Amount" :error-messages="payErrors.paidAmount" class="mb-2 flex-1-1" />
        </div>
        <div class="d-flex ga-2">
          <AppTextField v-model.number="payForm.principalComponent" type="number" label="Principal" class="mb-2 flex-1-1" />
          <AppTextField v-model.number="payForm.interestComponent" type="number" label="Interest" class="mb-2 flex-1-1" />
        </div>
        <AppAlert v-if="splitMismatch" type="warning" density="compact" class="mb-2">
          Principal + Interest ({{ formatCurrency(splitTotal) }}) must equal the EMI amount ({{ formatCurrency(payForm.paidAmount || 0) }}).
        </AppAlert>

        <AppSelect v-model="payForm.fundAccountKey" :items="fundAccountOptions" item-title="label" item-value="key" label="Payment From" :error-messages="payErrors.fundAccountKey" class="mb-2" />
        <AppSelect v-model="payForm.paymentModeId" :items="paymentModeOptions" item-title="name" item-value="id" label="Payment Mode" clearable class="mb-2" />
        <AppTextField v-model="payForm.referenceNumber" label="Reference Number" class="mb-2" />
