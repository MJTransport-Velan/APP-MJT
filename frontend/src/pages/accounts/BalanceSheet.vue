  ...
  <div>
    <div class="d-flex flex-wrap align-center justify-space-between mb-4 ga-2">
      <div>
        ...
        <p class="text-caption text-medium-emphasis mb-0">
          What MJ Transport owns, owes, and its net financial position — as of
          {{ formatDate(asOfDate) }}
        </p>
      </div>
    ...
    <template v-else-if="result">
      <AppAlert v-if="!result.reconciliation.reconciled" type="error" class="mb-4">
        Financial position does not reconcile — Assets ({{
          formatCurrency(result.totalAssets)
        }}) vs Liabilities + Equity ({{
          formatCurrency(result.totalLiabilities + result.totalEquity)
        }}), difference {{ formatCurrency(result.reconciliation.difference) }}.
      </AppAlert>
      ...
      <AppCard class="pa-4 mb-4 mt-4">
        ...
        <div class="bs-bar">
          <div
            class="bs-bar__seg"
            :style="{ width: liabilitiesPct + '%', background: 'var(--color-error)' }"
            :title="`Liabilities ${formatCurrency(result.totalLiabilities)}`"
          />
          <div
        ...
        <div class="d-flex ga-4 mt-2 text-caption">
          <span
            ><span class="bs-dot" style="background: var(--color-error)" />
            Liabilities</span
          >
          <span
            ><span class="bs-dot" style="background: var(--color-primary)" /> Equity</span
          >
        </div>
      </AppCard>
      ...
      <div class="row">
        <div class="col-12 col-md-6">
          <AppCard class="pa-4">
            ...
            >
              ...
              <span class="d-flex align-center ga-1">
                <span class="font-weight-medium">{{
                  formatCurrency(line.value(result))
                }}</span>
                <AppIcon
                  v-if="!line.disabled"
            ...
            <div
              class="d-flex justify-space-between font-weight-bold pt-2 mt-1 bs-total-row"
            >
          ...
          <AppCard class="pa-4 mt-4">
            ...
            <div class="bs-row bs-row--disabled">
              <span>Gross Block (original cost)</span
              ><span>{{ formatCurrency(result.assets.fixedAssets.grossBlock) }}</span>
            </div>
            <div class="bs-row bs-row--disabled">
              <span>Less: Accumulated Depreciation</span
              ><span
                >({{
                  formatCurrency(result.assets.fixedAssets.accumulatedDepreciation)
                }})</span
              >
            </div>
            <div
            ...
            >
              ...
              ><span>{{ formatCurrency(result.assets.fixedAssets.total) }}</span>
            </div>
            <div
              class="bs-row"
              @click="openSection('openingAssets', 'Existing / Opening Assets')"
            >
              <span class="d-flex align-center ga-2">
                <AppIcon
              ...
              <span class="d-flex align-center ga-1">
                <span class="font-weight-medium">{{
                  formatCurrency(result.assets.fixedAssets.openingAssets)
                }}</span>
                <AppIcon
                  icon="mdi-chevron-right"
                  size="small"
            ...
            <div class="bs-row" @click="openSection('newAssets', 'New Assets')">
              ...
              <span class="d-flex align-center ga-1">
                <span class="font-weight-medium">{{
                  formatCurrency(result.assets.fixedAssets.newAssets)
                }}</span>
                <AppIcon
                  icon="mdi-chevron-right"
                  size="small"
          ...
          <AppCard class="pa-4 mt-4">
            ...
            <div class="bs-comp-bar">
              <div
                v-for="seg in assetComposition"
                :key="seg.key"
                class="bs-comp-bar__seg"
            ...
            <div class="bs-legend mt-2">
              ...
              >
                <span
                  ><span class="bs-dot" :style="{ background: seg.color }" />
                  {{ seg.label }}</span
                >
        ...
        <div class="col-12 col-md-6">
          ...
          <AppCard class="pa-4 mt-4">
            ...
            >
              ...
              ><span>{{ formatCurrency(result.totalEquity) }}</span>
            </div>
          </AppCard>
    ...
    <AppDialog v-model="sectionDialog.open" max-width="640">
      <AppCardTitle class="d-flex justify-space-between align-center">
        ...
        <AppBtn
          ...
          variant="text"
          size="small"
          @click="sectionDialog.open = false"
        />
      </AppCardTitle>
      <AppCardText>
        <div v-for="(group, gi) in sectionDialog.groups" :key="gi" class="mb-4">
          ...
          <p v-if="group.rows.length === 0" class="text-caption text-medium-emphasis">
            No data.
          </p>
            ...
            <AppTable density="compact">
              ...
              <tbody>
                <tr
                  ...
                  @click="
                    group.partyType &&
                      openParty(group.partyType, row.id, row.name, sectionDialog.title)
                  "
                >
                  <td>{{ row.name }}</td>
    ...
    <AppDialog v-model="partyDialog.open" max-width="480">
      <AppCardTitle class="d-flex justify-space-between align-center">
        ...
        <AppBtn
          ...
          variant="text"
          size="small"
          @click="partyDialog.open = false"
        />
      </AppCardTitle>
      <AppCardText>
        <div v-if="partyDialog.loading" class="d-flex justify-center pa-4">
          <AppProgressCircular indeterminate color="primary" size="32" />
        </div>
        <template v-else>
          <div
            v-for="(row, i) in partyDialog.rows"
            :key="i"
          ...
          <p
            v-if="partyDialog.rows.length === 0"
...
const assetLines: SectionLine[] = [
  {
    key: "fixedAssetsVehicles",
    label: "Fixed Assets — Vehicles",
    icon: "mdi-truck-outline",
    value: (r) => r.assets.fixedAssets.vehicles,
  },
  {
    key: "fixedAssetsOther",
    label: "Fixed Assets — Equipment & Other",
    icon: "mdi-desktop-tower-monitor",
    value: (r) => r.assets.fixedAssets.equipmentAndOther,
  },
  {
    key: "cash",
    label: "Cash",
    icon: "mdi-cash",
    value: (r) => r.assets.currentAssets.cash,
  },
  {
    key: "bank",
    label: "Bank",
    icon: "mdi-bank-outline",
    value: (r) => r.assets.currentAssets.bank,
  },
  {
    key: "customerReceivables",
    label: "Receivables",
    icon: "mdi-account-cash-outline",
    value: (r) => r.assets.currentAssets.receivables,
  },
  {
    key: "advancesRecoverable",
    label: "Advances Recoverable",
    icon: "mdi-cash-fast",
    value: (r) => r.assets.currentAssets.advances,
  },
  // Money lent out to friends, relatives and anyone else with no master
  // record. Separate from Advances Recoverable on purpose: an advance is
  // paid against work still to come, this is simply owed back.
  {
    key: "loansGiven",
    label: "Loans & Advances Given",
    icon: "mdi-hand-coin-outline",
    value: (r) => r.assets.currentAssets.loansGiven,
  },
...
const liabilityLines: SectionLine[] = [
  {
    ...
    label: "Vehicle Loans",
    icon: "mdi-truck-outline",
    value: (r) => r.liabilities.vehicleLoans,
  },
  {
    key: "bankLoans",
    label: "Bank / Business Loans",
    icon: "mdi-bank-outline",
    value: (r) => r.liabilities.bankLoans,
  },
  {
    key: "ownerLoans",
    label: "Owner Loans",
    icon: "mdi-account-cash-outline",
    value: (r) => r.liabilities.ownerLoans,
  },
  {
    key: "otherLoans",
    label: "Other Loans",
    icon: "mdi-hand-coin-outline",
    value: (r) => r.liabilities.otherLoans,
  },
  {
    key: "supplierPayables",
    label: "Supplier Payables",
    icon: "mdi-truck-delivery-outline",
    value: (r) => r.liabilities.supplierPayables,
  },
  {
    key: "employeePayables",
    label: "Driver / Employee Payables",
    icon: "mdi-account-group-outline",
    value: (r) => r.liabilities.employeePayables,
  },
  {
    key: "customerAdvances",
    label: "Customer Advances",
    icon: "mdi-cash-refund",
    value: (r) => r.liabilities.customerAdvances,
  },
  ...
  {
    key: "openingUnclassified",
    label: "Owner Funds — Needs Review",
    icon: "mdi-help-circle-outline",
    value: (r) => r.liabilities.openingUnclassified,
  },
  {
    key: "taxPayables",
    label: "Tax / Statutory Payables",
    icon: "mdi-file-percent-outline",
    value: (r) => r.liabilities.taxPayables,
    disabled: true,
  },
  {
    ...
    value: (r) => r.liabilities.otherLiabilities,
  },
];
...
const equityLines: SectionLine[] = [
  {
    ...
    value: (r) => r.equity.ownerCapital,
  },
  {
    key: "openingEquity",
    label: "Opening Equity (brought forward)",
    icon: "mdi-database-import-outline",
    value: (r) => r.equity.openingEquityAdjustments,
  },
  {
    key: "retainedProfit",
    label: "Retained Profit",
    icon: "mdi-chart-line",
    value: (r) => r.equity.retainedProfit,
    disabled: true,
  },
  {
    key: "drawings",
...
const fyOptions = computed(() => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    opts.push({
      title: i === 0 ? `${fyLabel(startYear)} (current, to date)` : fyLabel(startYear),
      value: asOf,
    });
  }
  return opts;
...
const assetComposition = computed(() => {
  ...
  const items = [
    {
      key: "bankAndCash",
      label: "Bank & Cash",
      value: a.currentAssets.cash + a.currentAssets.bank,
      color: "var(--color-primary)",
    },
    {
      key: "customerReceivables",
      label: "Receivables",
      value: a.currentAssets.receivables,
      color: "var(--color-secondary)",
    },
    {
      key: "advancesRecoverable",
      label: "Advances Recoverable",
      value: a.currentAssets.advances,
      color: "var(--color-success)",
    },
    {
      key: "loansGiven",
      label: "Loans & Advances Given",
      value: a.currentAssets.loansGiven,
      color: "var(--color-info)",
    },
    {
      key: "fixedAssets",
      label: "Fixed Assets",
      value: a.fixedAssets.total,
  ...
  return items
    .filter((i) => i.value > 0.004)
    .map((i) => ({ ...i, pct: (i.value / total) * 100 }));
});

// --- Section breakdown dialog ---
const sectionDialog = ref<{ open: boolean; title: string; groups: SectionGroup[] }>({
  open: false,
  title: "",
  groups: [],
});
...
function assetGroups(
  rows: { id: string; name: string; amount: number; origin: string }[],
  label: string
): SectionGroup[] {
  const opening = rows
    .filter((a) => a.origin === "OPENING")
    .map((a) => ({ id: a.id, name: a.name, amount: a.amount }));
  const fresh = rows
    .filter((a) => a.origin !== "OPENING")
    .map((a) => ({ id: a.id, name: a.name, amount: a.amount }));
  ...
  if (opening.length)
    groups.push({
      title: `${label} — Existing / Opening`,
      partyType: null,
      rows: opening,
    });
  groups.push({
    title: opening.length ? `${label} — New` : label,
    partyType: null,
    rows: fresh,
  });
  return groups;
...
function sectionGroups(key: string): SectionGroup[] {
  ...
  switch (key) {
      ...
      return [
        {
          title: "Bank Accounts",
          partyType: null,
          rows: b.bankAccounts.map((a) => ({
            id: a.id,
            name: a.bankName ? `${a.name} — ${a.bankName}` : a.name,
    ...
    case "customerReceivables":
      return [
        { title: "Customer-wise", partyType: "customer", rows: b.customerReceivables },
      ];
    case "advancesRecoverable":
      return [
        {
          title: "Supplier Advances Paid",
          partyType: "supplier",
          rows: b.supplierAdvances,
        },
    ...
    // No partyType: the person lent to is free text, not a master record, so
    // there is no party ledger to drill into.
    case "loansGiven":
      return [{ title: "Person-wise", partyType: null, rows: b.loansGiven }];
    ...
    case "fixedAssetsVehicles":
      return assetGroups(
        b.fixedAssets.filter((a) => a.category === "Vehicle"),
        "Vehicle Assets"
      );
    ...
    case "openingAssets":
      return [
        {
          title: "Carried over from the previous system",
          partyType: null,
          rows: b.openingAssetRows.map((a) => ({
            id: a.id,
            name: a.name,
            amount: a.amount,
          })),
    ...
    case "newAssets":
      return [
        {
          title: "Bought through this system",
          partyType: null,
          rows: b.newAssetRows.map((a) => ({ id: a.id, name: a.name, amount: a.amount })),
        },
      ];
    case "otherAssets":
      return [
        { title: "Opening adjustments", partyType: null, rows: b.openingOtherAssets },
      ];
      ...
      return [
        {
          ...
          partyType: null,
          rows: b.openingOtherLiabilities,
        },
      ];
    case "openingEquity":
      return [
        { title: "Opening adjustments", partyType: null, rows: b.openingOtherEquity },
      ];
    case "openingUnclassified":
      return [
        {
          title: "Owner funds still to classify",
          partyType: null,
          rows: b.openingUnclassifiedOwnerFunds,
        },
      ];
    case "supplierPayables":
      return [
        { title: "Supplier-wise", partyType: "supplier", rows: b.supplierPayables },
      ];
    ...
    case "customerAdvances":
      return [
        { title: "Customer-wise", partyType: "customer", rows: b.customerAdvances },
      ];
    case "vehicleLoans":
    case "bankLoans":
    case "ownerLoans":
    case "otherLoans": {
      ...
      const groups: SectionGroup[] = [
        {
          ...
          rows: b.loans
            ...
            .map((l) => ({
              id: l.id,
              name: l.linkedTo
                ? `${l.name} — ${l.lenderName} (${l.linkedTo})`
                : `${l.name} — ${l.lenderName}`,
              amount: l.amount,
            })),
        },
      ...
      if (key === "ownerLoans") {
        groups.push({
          title: "Owner Loans recorded on Capital & Owner Funds",
          partyType: "capitalPartner",
          rows: b.ownerLoans,
        });
      }
      return groups;
    ...
    case "drawings":
      return [
        {
          title: "Partner-wise (net capital)",
          partyType: "capitalPartner",
          rows: b.capitalAccount,
        },
      ];
    default:
      return [];
...
async function onExport() {
  ...
  try {
    ...
    const rows: Record<string, unknown>[] = [
      {
        section: "ASSETS",
        particulars: "Fixed Assets — Vehicles",
        amount: r.assets.fixedAssets.vehicles,
      },
      {
        section: "ASSETS",
        particulars: "Fixed Assets — Equipment & Other",
        amount: r.assets.fixedAssets.equipmentAndOther,
      },
      {
        section: "ASSETS",
        particulars: "Total Fixed Assets",
        amount: r.assets.fixedAssets.total,
      },
      ...
      { section: "ASSETS", particulars: "Bank", amount: r.assets.currentAssets.bank },
      {
        ...
        amount: r.assets.currentAssets.receivables,
      },
      {
        section: "ASSETS",
        particulars: "Advances Recoverable",
        amount: r.assets.currentAssets.advances,
      },
      {
        section: "ASSETS",
        particulars: "Loans & Advances Given",
        amount: r.assets.currentAssets.loansGiven,
      },
      {
        section: "ASSETS",
        particulars: "Total Current Assets",
        amount: r.assets.currentAssets.total,
      },
      ...
      {
        section: "LIABILITIES",
        particulars: "Vehicle Loans",
        amount: r.liabilities.vehicleLoans,
      },
      {
        section: "LIABILITIES",
        particulars: "Bank / Business Loans",
        amount: r.liabilities.bankLoans,
      },
      {
        section: "LIABILITIES",
        particulars: "Owner Loans",
        amount: r.liabilities.ownerLoans,
      },
      {
        section: "LIABILITIES",
        particulars: "Other Loans",
        amount: r.liabilities.otherLoans,
      },
      {
        section: "LIABILITIES",
        particulars: "Supplier Payables",
        amount: r.liabilities.supplierPayables,
      },
      {
        section: "LIABILITIES",
        particulars: "Driver / Employee Payables",
        amount: r.liabilities.employeePayables,
      },
      {
        section: "LIABILITIES",
        particulars: "Customer Advances",
        amount: r.liabilities.customerAdvances,
      },
      {
        section: "LIABILITIES",
        particulars: "Tax / Statutory Payables",
        amount: r.liabilities.taxPayables,
      },
      {
        section: "LIABILITIES",
        particulars: "Other Liabilities",
        amount: r.liabilities.otherLiabilities,
      },
      {
        section: "LIABILITIES",
      ...
      {
        section: "EQUITY",
        particulars: "Retained Profit",
        amount: r.equity.retainedProfit,
      },
      {
        section: "EQUITY",
        particulars: "Drawings (deduction)",
        amount: -r.equity.drawings,
      },
