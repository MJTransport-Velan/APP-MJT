<template>
  <div>
    <div class="d-flex flex-wrap align-center justify-space-between mb-4 ga-2">
      <div>
        ...
        <p class="text-caption text-medium-emphasis mb-0">
          Money you lent out — to a friend, a relative, or anyone else — that is coming
          back. It leaves the Bank/Cash account you choose and is held as an asset until
          it is repaid; it is never counted as an expense.
        </p>
      </div>
      <AppBtn
        v-if="canCreate"
    ...
    <AppAlert
      v-if="summary && summary.writtenOffTotal > 0"
      type="warning"
      variant="tonal"
      density="compact"
      class="mb-4"
    >
      {{ formatCurrency(summary.writtenOffTotal) }} has been written off and is no longer
      counted as an asset on the Balance Sheet.
    </AppAlert>

    <div class="d-flex flex-wrap ga-2 mb-3 mt-4">
    ...
    >
      <template #item.partyName="{ item }">
        <div class="d-flex align-center ga-2">
          ...
          <AppChip
            v-if="(item as any).origin === 'OPENING'"
            size="x-small"
            variant="tonal"
            color="secondary"
          >
        ...
        <div class="text-caption text-medium-emphasis">
          ...
          }}<span v-if="(item as any).partyContact">
            · {{ (item as any).partyContact }}</span
          >
        </div>
      </template>
      <template
        #item.amount="{ item }"
        >{{ formatCurrency((item as any).amount) }}</template
      >
      <template
        #item.repaid="{ item }"
        >{{ formatCurrency((item as any).totals.repaid) }}</template
      >
      <template #item.outstanding="{ item }">
        <span
          class="font-weight-medium"
          >{{ formatCurrency((item as any).totals.outstanding) }}</span
        >
      </template>
      <template
        #item.givenDate="{ item }"
        >{{ formatDate((item as any).givenDate) }}</template
      >
      <template #item.expectedReturnDate="{ item }">
        <span
          v-if="(item as any).expectedReturnDate"
          :class="(item as any).totals.isOverdue ? 'text-error' : ''"
        >
      ...
      <template #item.status="{ item }">
        <AppChip
          size="small"
          variant="tonal"
          :color="statusColor(item as any)"
          >{{ statusLabel(item as any) }}</AppChip
        >
      </template>
      <template #item.actions="{ item }">
        ...
        <template v-if="canEdit">
          ...
          <AppBtn
            ...
            size="small"
            title="Edit"
            @click="openFormDialog(item as any)"
          />
        </template>
        <AppBtn
          v-if="canDelete"
    ...
    >
      <AppAlert type="info" variant="tonal" density="compact" class="mb-3">
        The amount leaves the Bank/Cash account below and is held as an asset until it
        comes back. Nothing here is treated as an expense, so it does not affect Profit
        &amp; Loss.
      </AppAlert>
      <AppTextField
        v-model="form.partyName"
        label="Given To"
        :error-messages="errors.partyName"
        class="mb-2"
      />
      <AppTextField
        v-model="form.partyContact"
    ...
    >
      <p class="text-caption text-medium-emphasis mb-3">
        ...
        {{ actionTarget?.referenceNumber }}. The amount below is added back to the account
        you choose.
      </p>
      <AppTextField
      ...
      />
      <AppTextField
        v-model="repaymentForm.referenceNumber"
    ...
    >
      <AppAlert type="warning" variant="tonal" density="compact" class="mb-3">
        Writing off {{ formatCurrency(actionTarget?.totals.outstanding || 0) }} means you
        no longer expect it back. No Bank or Cash balance moves — the money already left —
        but it stops counting as an asset on the Balance Sheet. You can reopen it later.
      </AppAlert>
      <AppTextarea
    ...
    <AppDialog v-model="detailDialog" max-width="640">
      <AppCard>
        <AppCardTitle class="text-h6"
          >Repayments — {{ actionTarget?.partyName }}</AppCardTitle
        >
        <AppCardText>
          <div class="d-flex flex-wrap ga-4 mb-3">
            <div>
              <span class="text-caption text-medium-emphasis">Lent</span>
              <div class="font-weight-medium">
                {{ formatCurrency(actionTarget?.amount || 0) }}
              </div>
            </div>
            <div>
              <span class="text-caption text-medium-emphasis">Repaid</span>
              <div class="font-weight-medium">
                {{ formatCurrency(actionTarget?.totals.repaid || 0) }}
              </div>
            </div>
            <div>
              <span class="text-caption text-medium-emphasis">Still Owed</span>
              <div class="font-weight-medium">
                {{ formatCurrency(actionTarget?.totals.outstanding || 0) }}
              </div>
            </div>
          </div>
          <p
            v-if="!actionTarget?.repayments.length"
              ...
              <tbody>
                <tr v-for="repayment in actionTarget.repayments" :key="repayment.id">
                  ...
                  <td class="text-caption">
                    {{ repayment.referenceNumber || repayment.remarks || "—" }}
                  </td>
          ...
          <p
            v-if="actionTarget?.writtenOffReason"
            class="text-caption text-warning mt-3 mb-0"
          >
